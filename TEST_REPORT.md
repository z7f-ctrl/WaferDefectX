...
### 6.2 端到端延迟（含 ONNX 推理，估算）

| 组件 | 延迟 |
|------|------|
| C++ 预处理+定位+特征 | ~6 ms |
| ONNX Runtime 分类（RF） | ~1 ms |
| **总计** | **~7 ms** |
...
| 4 | C++ 路径不含 ONNX 推理 | 端到端需 Python | P3 规划中 |

---

## 8. 结论

WaferDefectX 在 WM-811K 真实数据集上通过 ResNet-18 ROI 分类器达到 **93.83% 准确率**，超过 90% 目标。scratch 召回率从 RF 的 44% 提升至 90%，Macro F1 从 0.67 提升至 0.93。所有单元测试（13/13）和冒烟测试（5/5）通过，C++ 编译零 warning，特征对齐差异 <0.01。

项目已具备生产候选状态，后续可聚焦于：
1. 完整数据集（172,950 张）训练以进一步提升性能
2. ONNX 导出与 C++ 端到端集成
3. 产线部署与实时推理优化
